-- Option B Phase 2 — Trader resale royalty engine (rules 1–3)
-- Apply after 20250625100012_phase1_wallet_customer.sql

-- ---------------------------------------------------------------------------
-- 1. Additive order_items columns
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 2. Royalty decision helpers
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public._infer_product_kind(
  p_description TEXT,
  p_seller_role TEXT
)
RETURNS TEXT
...
BEGIN
  IF p_description IS NOT NULL AND btrim(p_description) <> '' THEN
    BEGIN
      ...
      v_kind := v_meta->>'product_kind';
      IF v_kind IN ('raw_farmer', 'trader_relist', 'processed') THEN
        RETURN v_kind;
      END IF;
      IF v_meta ? 'processed_product_id' OR v_kind = 'processed' THEN
        RETURN 'processed';
      END IF;
      IF v_meta ? 'source_order_item_id' THEN
        RETURN 'trader_relist';
      END IF;
  ...
  IF public._is_trader_role(p_seller_role) THEN
    RETURN 'trader_relist';
  END IF;
  RETURN 'raw_farmer';
END;
...
REVOKE ALL ON FUNCTION public._infer_product_kind(TEXT, TEXT) FROM PUBLIC;

CREATE OR REPLACE FUNCTION public._resolve_sale_royalty_mode(
  p_buyer_role TEXT,
  p_seller_role TEXT,
  p_product_kind TEXT DEFAULT 'raw_farmer'
)
RETURNS TEXT
LANGUAGE sql IMMUTABLE AS $$
  SELECT CASE
    WHEN COALESCE(p_product_kind, '') = 'processed' THEN 'deferred_settle'
    WHEN NOT public._buyer_participates_in_royalty_chain(p_buyer_role) THEN 'none'
    WHEN public._is_trader_role(p_seller_role)
         AND COALESCE(p_buyer_role, '') = 'industrialist' THEN 'immediate'
    ELSE 'none'
  END;
$$;

REVOKE ALL ON FUNCTION public._resolve_sale_royalty_mode(TEXT, TEXT, TEXT) FROM PUBLIC;
...
DECLARE
  ...
  v_pct NUMERIC;
BEGIN
  IF p_description IS NULL OR btrim(p_description) = '' THEN RETURN '{}'::jsonb; END IF;
  BEGIN
    v_meta := p_description::jsonb;
  EXCEPTION WHEN OTHERS THEN
    RETURN '{}'::jsonb;
  ...
  RETURN jsonb_build_object(
    'product_kind', COALESCE(v_meta->>'product_kind', 'raw_farmer'),
    ...
    'source_order_item_id', v_meta->>'source_order_item_id',
    'processed_product_id', v_meta->>'processed_product_id',
    'royalty_obligation_id', v_meta->>'royalty_obligation_id',
    'source_batch_id', v_meta->>'source_batch_id'
  );
END;
$$;
...
LANGUAGE sql IMMUTABLE AS $$
  SELECT COALESCE(p_existing, '[]'::jsonb)
    || jsonb_build_array(
      jsonb_build_object('user_id', p_seller_id, 'role', p_seller_role, 'acquired_at', to_jsonb(p_acquired_at)),
      jsonb_build_object('user_id', p_buyer_id, 'role', p_buyer_role, 'acquired_at', to_jsonb(p_acquired_at))
    );
$$;
...
BEGIN
  ...
  IF v_apply_royalty THEN
    ...
    PERFORM public._wallet_ledger_entry(p_buyer_id, 'purchase', -p_item_total, p_order_id, 'Purchase: ' || p_product_name);
    PERFORM public._wallet_ledger_entry(p_seller_id, 'sale_income', v_seller_net, p_order_id, 'Sale proceeds: ' || p_product_name);
    PERFORM public._wallet_ledger_entry(p_original_farmer_id, 'royalty_income', v_royalty, p_order_id, 'Royalty (' || COALESCE(p_royalty_percent, 12.5) || '%): ' || p_product_name);
    PERFORM public._wallet_ledger_entry(p_seller_id, 'royalty_paid', -v_royalty, p_order_id, 'Royalty remittance for ' || p_product_name);
  ELSE
...
-- ---------------------------------------------------------------------------
-- 3. checkout_order — Option B rules 1–3
-- ---------------------------------------------------------------------------
