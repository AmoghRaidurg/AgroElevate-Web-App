-- PRODUCTION Phase F0 — 015 v2: Commerce E2E fixes (PostgreSQL-compatible)
-- Apply after 20250625100014_phase3_manufacturing_royalty.sql
-- Supersedes: 20250625100015_prod_commerce_e2e_fix.sql (invalid %ROWTYPE parameter)
--
-- Fixes:
...
--    Parameter: UUID (PostgreSQL does not allow %ROWTYPE in signatures)
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public._resolve_crop_id_for_product(p_product_id UUID)
RETURNS UUID
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  IF v_product.description IS NOT NULL AND btrim(v_product.description) <> '' THEN
    BEGIN
      v_meta := v_product.description::jsonb;
  ...
  FROM public.crops c
  WHERE c."farmerId" = v_product.seller_id::text
    AND c.name = v_product.name
  ...
  SELECT name INTO v_farmer_name FROM public.profiles WHERE id = v_product.seller_id;
  v_category := COALESCE(NULLIF(v_product.crop_type, ''), 'General');
  ...
  INSERT INTO public.crops (
    ...
    v_product.name,
    GREATEST(v_product.quantity::numeric, 0),
    COALESCE(v_product.unit, 'kg'),
    COALESCE(v_product.price_per_unit, 0),
    CURRENT_DATE::text,
    jsonb_build_object('product_id', v_product.id::text, 'source', 'checkout_bridge')::text,
    v_category,
    '',
    'available',
    0,
...
REVOKE ALL ON FUNCTION public._resolve_crop_id_for_product(UUID) FROM PUBLIC;
